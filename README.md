...
Play four drums and four strokes, with up to eight players per note. Drum hits
are synthesized from the physical model; the optional room reverb uses three
bundled third-party impulse responses documented in
[THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md).

**VST3 · CLAP · Standalone** on macOS, Windows and Linux · **Audio Unit** on macOS.

<!-- distribution-link-begin -->
**[Download latest distribution](https://github.com/protocodus/virtual-instrument-taikor/actions/runs/34822139048/artifacts/10339356602)** — build 20, built from [`7fc58ab18c14`](https://github.com/protocodus/virtual-instrument-taikor/commit/7fc58ab18c1441f428927874eb0339e27c2d6be4).
...
On macOS, use the PKG installer or copy the bundles from the ZIP. The current
CI distribution uses ad-hoc bundle signatures, an unsigned installer and no
Apple notarization; these are commercial-release blockers. See the
[commercial release readiness checklist](Docs/commercial-release-readiness-2026-09-14.md).
...
Thirty host parameters. Performer is a persisted, non-automatable
performance identity; the other twenty-nine are automatable and describe the
...
| --- | --- | ---: | ---: | ---: |
| `01-stroke-vocabulary.wav` | All four strokes on the o-daiko, in keyboard order | 4.2 s | −17.8 dBFS | +14.8 dB |
| `02-the-four-drums.wav` | A Don on each of the four drums: o-daiko, nagado-daiko, okedo, shime | 6.3 s | −25.8 dBFS | +22.8 dB |
| `03-the-playing-grid.wav` | The whole grid: four strokes on each of the four drums | 10.1 s | −17.8 dBFS | +14.8 dB |
| `04-drums-ka.wav` | An edge-of-head stroke on each of the four drums | 3.9 s | −23.1 dBFS | +20.1 dB |
| `05-drums-rim-shot.wav` | A rim shot on each of the four drums | 5.1 s | −17.7 dBFS | +14.7 dB |
| `06-odaiko-phrase.wav` | A phrase on the o-daiko | 10.0 s | −18.2 dBFS | +15.2 dB |
| `07-chudaiko-phrase.wav` | The same phrase on the nagado-daiko | 6.7 s | −20.5 dBFS | +17.5 dB |
| `08-okedo-phrase.wav` | The same phrase on the okedo-daiko | 5.1 s | −20.1 dBFS | +17.1 dB |
| `09-shime-phrase.wav` | The same phrase on the shime-daiko | 4.2 s | −24.5 dBFS | +21.5 dB |
| `10-velocity-dynamics.wav` | One stroke from a ghost note to a full-arm hit | 5.9 s | −28.9 dBFS | +25.9 dB |
| `11-rolls-and-presses.wav` | Press rolls, a played flam and a roll accelerating into a rim shot | 5.4 s | −16.2 dBFS | +13.2 dB |
| `12-bachi-hardness.wav` | Felt beater through to a hard oak bachi | 4.8 s | −23.6 dBFS | +20.6 dB |
| `13-strike-position.wav` | The same stroke walked from centre to rim | 4.6 s | −21.7 dBFS | +18.7 dB |
| `14-head-tension.wav` | Slack head through to fully tacked | 5.5 s | −24.2 dBFS | +21.2 dB |
| `15-head-material.wav` | Thin synthetic film through to thick cowhide | 5.5 s | −29.1 dBFS | +26.1 dB |
| `16-shell-material.wav` | Light laminated staves through to dense carved zelkova, on a rim shot | 5.4 s | −28.7 dBFS | +25.7 dB |
| `17-air-coupling.wav` | Open body through to a fully sealed one | 5.5 s | −23.7 dBFS | +20.7 dB |
| `18-body-depth.wav` | Shallow body through to deep | 5.5 s | −29.6 dBFS | +26.6 dB |
| `19-head-damping.wav` | Open head through to heavily damped | 6.3 s | −27.8 dBFS | +24.8 dB |
| `20-octave-body.wav` | The keyboard as one drum retuned four times, then as the four drums | 7.8 s | −24.9 dBFS | +21.9 dB |
| `21-mic-distance.wav` | The close pair from 3 cm out to 40 cm, on the head and then on the body | 8.5 s | −14.5 dBFS | +11.5 dB |
| `22-mic-spread.wav` | The close pair from coincident to fully opened | 4.5 s | −23.1 dBFS | +20.1 dB |
| `23-hand-damping.wav` | A hand laid on a ringing head, from MIDI CC1 | 6.0 s | −28.4 dBFS | +25.4 dB |
| `24-pitch-wheel.wav` | The wheel pressing the head sharp and flat | 4.3 s | −28.9 dBFS | +25.9 dB |
| `25-ensemble-piece.wav` | A longer piece moving between all four drums of the grid | 10.4 s | −18.7 dBFS | +15.7 dB |
| `26-polar-strikes.wav` | Fixed left, centre and right strikes, then one circuit around the head | 8.6 s | −24.7 dBFS | +21.7 dB |
| `27-performer-ensemble.wav` | Phase-locked P1 copies, then the same phrase layered as P1-P4 | 5.7 s | −22.3 dBFS | +19.3 dB |
<!-- peaks-table-end -->
...
terms — see `THIRD_PARTY_NOTICES.md`. The distribution also embeds the three
room impulse responses listed there. Their source notices and supplied licence
files are included in packages, but they are not relicensed under Taikor's MIT
license. EchoThief redistribution permission and compliance with the Voxengo
redistribution conditions must be verified before a commercial release; this
repository does not certify legal clearance. The demonstration audio is
generated by this repository's own code.

The [commercial release readiness checklist](Docs/commercial-release-readiness-2026-09-14.md)
records the remaining signing, licensing, platform and host-validation gates.
